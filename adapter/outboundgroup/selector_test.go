...
func newTestSelector(t *testing.T, option *GroupCommonOption, selectorOption SelectorOption, proxies ...C.Proxy) *Selector {
	t.Helper()

	if option.ExpectedStatus != "" && selectorOption.expectedStatus == nil {
		expectedStatus, err := utils.NewUnsignedRanges[uint16](option.ExpectedStatus)
		require.NoError(t, err)
		selectorOption.expectedStatus = expectedStatus
	}

	hc := provider.NewHealthCheck(proxies, option.URL, 0, 0, true, selectorOption.expectedStatus)
	pd, err := provider.NewCompatibleProvider(option.Name+"-provider", proxies, hc)
	require.NoError(t, err)

	selector, err := NewSelector(*option, selectorOption, newStubProxy("COMPATIBLE", option.URL, 0, true), []P.ProxyProvider{pd})
	require.NoError(t, err)
...
func TestSelectorUsesDefault(t *testing.T) {
	...
	group := newTestSelector(t, &GroupCommonOption{
		Name: "manual",
	}, SelectorOption{Default: "Second"}, first, second)
...
func TestSelectorExactDefaultDoesNotProbe(t *testing.T) {
	...
	group := newTestSelector(t, &GroupCommonOption{
		Name: "manual",
	}, SelectorOption{Default: "proxy-node-2"}, first, second)
...
func TestSelectorDefaultPrefixDoesNotProbeBeforeSelection(t *testing.T) {
	...
	group := newTestSelector(t, &GroupCommonOption{
		Name: "manual",
	}, SelectorOption{Default: "proxy-node"}, first, second)
...
func TestSelectorDefaultPrefixUsesFirstReachableCandidate(t *testing.T) {
	...
	}, SelectorOption{Default: "proxy-node"}, first, second, third)
...
func TestSelectorDefaultWildcardUsesFirstReachableCandidate(t *testing.T) {
	...
	group := newTestSelector(t, &GroupCommonOption{
		Name: "manual",
	}, SelectorOption{Default: "proxy-node-hk-*"}, first, second, third)
	group.StartDefaultSelection()
...
func TestSelectorDefaultWildcardQuestionMarkDoesNotUsePrefixSemantics(t *testing.T) {
	...
	group := newTestSelector(t, &GroupCommonOption{
		Name: "manual",
	}, SelectorOption{Default: "proxy?"}, prefixOnly, matched, unmatched)
	group.StartDefaultSelection()
...
func TestSelectorDefaultSkipsNilProxy(t *testing.T) {
	...
	group := newTestSelector(t, &GroupCommonOption{
		Name: "manual",
	}, SelectorOption{Default: "proxy-node"}, nilProxy, first)
	group.StartDefaultSelection()
...
func TestSelectorDefaultFallsBackToFirstProxy(t *testing.T) {
	...
	group := newTestSelector(t, &GroupCommonOption{
		Name: "manual",
	}, SelectorOption{Default: "Missing"}, first, second)
