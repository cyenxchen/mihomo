...
func newTestURLTest(t *testing.T, option *GroupCommonOption, urlTestOption URLTestOption, proxies ...C.Proxy) *URLTest {
	t.Helper()
	...
	urlTest, err := NewURLTest(*option, urlTestOption, newStubProxy("COMPATIBLE", option.URL, 0, true), []P.ProxyProvider{pd})
	require.NoError(t, err)

	return urlTest
}

func TestNewURLTestPolicyPriority(t *testing.T) {
	option := GroupCommonOption{
		Name: "auto",
		URL:  "https://example.com",
	}
	urlTestOption := URLTestOption{
		Tolerance:      150,
		PolicyPriority: "Premium:0.1;Hong Kong:0.2;",
	}

	group, err := NewURLTest(option, urlTestOption, newStubProxy("COMPATIBLE", option.URL, 0, true), nil)
	require.NoError(t, err)
...
func TestParsePolicyPriorityInvalid(t *testing.T) {
	testCases := []struct {
		name           string
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			_, err := parsePolicyPriority(testCase.policyPriority)
...
func TestURLTestUsesWeightedDelay(t *testing.T) {
	...
	group := newTestURLTest(t, &GroupCommonOption{
		Name: "auto",
		URL:  testURL,
	}, URLTestOption{PolicyPriority: "Premium:0.1;"}, regular, premium)
...
func TestURLTestUsesSmallestMatchingFactor(t *testing.T) {
	...
	group := newTestURLTest(t, &GroupCommonOption{
		Name: "auto",
		URL:  testURL,
	}, URLTestOption{PolicyPriority: "Premium:0.9;Hong Kong:0.2;"}, regular, target)
...
func TestURLTestToleranceUsesWeightedDelay(t *testing.T) {
	...
	group := newTestURLTest(t, &GroupCommonOption{
		Name: "auto",
		URL:  testURL,
	}, URLTestOption{Tolerance: 10, PolicyPriority: "Premium:0.7;"}, current, premium)
...
func TestURLTestSwitchesWhenWeightedDelayBeatsTolerance(t *testing.T) {
	...
	group := newTestURLTest(t, &GroupCommonOption{
		Name: "auto",
		URL:  testURL,
	}, URLTestOption{Tolerance: 10, PolicyPriority: "Premium:0.5;"}, current, premium)
	group.fastNode = current
	group.fastSingle.Reset()
...
func TestURLTestDeadProxyDoesNotWinByWeight(t *testing.T) {
	...
	group := newTestURLTest(t, &GroupCommonOption{
		Name: "auto",
		URL:  testURL,
	}, URLTestOption{PolicyPriority: "Premium:0.1;"}, deadPremium, regular)
